...
class _PropertyFormScreenState extends State<PropertyFormScreen> {
    ...
    if (data['extra_data'] != null) {
      try {
        ...
        if (extra['photos'] != null) {
          _photos = List<String>.from(extra['photos']);
        }
  ...
  Future<void> _pickMultiImages() async {
    ...
    for (var image in images) {
      await _saveImageToAppDirectory(image.path);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            widget.editData == null ? 'အိမ်ခြံမြေ အသစ်ထည့်ရန်' : 'အိမ်ခြံမြေ ပြင်ဆင်ရန်', 
                          ...
                          return Stack(
                            children: [
                              Container(
                                ...
                                child: imageProvider == null ? const Icon(Icons.image, color: Colors.grey) : null,
                              ),
                              Positioned(
                                top: 4, 
                                right: 12, 
